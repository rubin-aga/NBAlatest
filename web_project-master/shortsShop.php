...
if (isset($_POST["add"])) {
    $check = 0;
    foreach ($_SESSION['cart'] as $cart_item) { // loop the items
    ...
    echo '<script>window.location="shortsShop.php"</script>';
}
...
<body>
<div class="header">
    <h2 class="logo zoomIn" ; style="text-transform: none;">NBAlatest - Shorts</h2>
...
<?php
$query = "SELECT * FROM shorts ORDER BY id ASC ";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo '<td>
            <div class="card">
            <img src="img/shorts/' . $row['name'] . '.png" alt="ks">
            <div class = "container">
                <h2>' . $row['actual_name'] . '  $' . $row['price'] . '</h2>
                <form method="post" action="shortsShop.php?action=add&id=" ' . $row["id"] . ' >
        ...
        if (!empty($_SESSION["cart"])) {
            ...
            foreach ($_SESSION["cart"] as $key => $value) {
                ...
                <tr>
                    ...
                    <td><a href="shortsShop.php?action=delete&hidden_id=<?php echo $value["hidden_id"]; ?>"><span
